...
from .scripts.utils_train import get_model, make_dataset_from_df_eval
...
def load_multi_table(data_dir, verbose=True):
    ...
    for table, meta in dataset_meta["tables"].items():
        if os.path.exists(os.path.join(data_dir, "train_with_id.csv")):
            train_df = pd.read_csv(os.path.join(data_dir, "train_with_id.csv"))
        else:
...
def model_gradient(
    ...
    dataset = make_dataset_from_df_eval(
        df,
        T,
        ...
        label_encoders=pretrained_model["label_encoders"],
    ...
    diffusion = pretrained_model["diffusion"]
    diffusion.eval()
    
    # print(diffusion.num_numerical_features)
    trainer = Trainer(
        diffusion,
        train_loader,
        lr=0,
