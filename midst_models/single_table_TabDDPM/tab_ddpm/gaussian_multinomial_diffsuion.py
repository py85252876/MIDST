...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def updated_loss(self, init_x, out_dict, t):
        b = init_x.shape[0]
        device = init_x.device

        assert b == 1, "Batch size must be 1 for this setup"
        ...
        # # 将当前时间步的损失存储到列表中
        # loss_multi_list.append(loss_multi.mean())
        # loss_gauss_list.append(loss_gauss.mean())


        # # # 将 t_list 转换为矩阵，形状为 [len(t_list), 1]
        # x = init_x

        # t_batch = t_list

        # # 构造单样本的数值特征和类别特征，扩展到 100 个时间步
        # x_num = x[:, :self.num_numerical_features].repeat(len(t_list), 1)  # [100, num_numerical_features]
        # x_cat = x[:, self.num_numerical_features:].repeat(len(t_list), 1)  # [100, num_categorical_features]

        # # 概率分布
        # pt_batch = torch.ones_like(t_batch).float() / self.num_timesteps  # [100]

        # # 对数值特征扩散
        # if x_num.shape[1] > 0:
        #     noise = torch.randn_like(x_num)  # [100, num_numerical_features]
        #     x_num_t = self.gaussian_q_sample(x_num, t_batch, noise=noise)  # [100, num_numerical_features]
        # else:
        #     x_num_t = x_num

        # # 对类别特征扩散
        # if x_cat.shape[1] > 0:
        #     log_x_cat = index_to_log_onehot(x_cat.long(), self.num_classes)  # [100, num_classes]
        #     log_x_cat_t = self.q_sample(log_x_start=log_x_cat, t=t_batch)  # [100, num_classes]
        # else:
        #     log_x_cat_t = x_cat

        # # 构造模型输入
        # x_in = torch.cat([x_num_t, log_x_cat_t], dim=1)  # [100, input_features]

        # # 模型前向传播
        # model_out = self._denoise_fn(x_in, t_batch)  # [100, output_features]

        # # 分割模型输出
        # model_out_num = model_out[:, :self.num_numerical_features]  # [100, num_numerical_features]
        # model_out_cat = model_out[:, self.num_numerical_features:]  # [100, num_categorical_features]

        # out_dict = None
        # if x_cat.shape[1] > 0:
        #     loss_multi = self._multinomial_loss(
        #         model_out_cat, log_x_cat, log_x_cat_t, t_batch, pt_batch, out_dict
        #     )  
        # else:
        #     loss_multi = torch.zeros_like(t_batch)

        # if x_num.shape[1] > 0:
        #     loss_gauss = self._gaussian_loss(
        #         model_out_num, x_num_t, x_num, t_batch, noise
        #     )  
        # else:
        #     loss_gauss = torch.zeros_like(t_batch)
