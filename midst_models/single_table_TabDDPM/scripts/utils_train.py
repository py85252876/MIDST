...
def make_dataset_from_df_eval(df, T, is_y_cond, ratios=[0.7, 0.2, 0.1], df_info=None, y_info  = None, label_encoders=None,num_transform = None,column_orders = None, std=0):
    """
    The order of the generated dataset: (y, X_num, X_cat)
    ...
    # encoder_list = ["type","operation","k_symbol","bank"]
    ...
    if X_cat is not None and len(df_info["cat_cols"]) > 0:
        X_cat_all = np.vstack((X_cat["train"]))
        X_cat_converted = []
        for col_index in range(X_cat_all.shape[1]):
            label_encoder = label_encoders[col_index]
            X_cat_converted.append(
                label_encoder.transform(X_cat_all[:, col_index])
            )
            if std > 0:
                # add noise
                X_cat_converted[-1] += np.random.normal(
    ...
    return transform_dataset(D, T, None, num_transform)
